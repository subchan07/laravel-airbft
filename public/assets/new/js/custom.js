...
(function ($) {
	...
	*===================================*/
	...
	if (zoomActive) {
		if ($(window).width() >= 768) {
			var firstImgHeight = $(".products-detail-section .product_img").height();
			var divWidth = $(".products-detail-section .product_img").width();
		...
		callbacks: {
			open: function () {
				...
				if ($(window).width() >= 768) {
					...
					var divWidth = $(".quickview-popup .product_img").width();
...
function getAccordion(element_id, screen) {
	...
	if ($(window).width() < screen) {
		...
		jQuery.each(obj_tabs, function (n, val) {
			...
			concat += '<h4 class="panel-title"><a id="predict' + n + '" role="button" data-bs-toggle="collapse" data-parent="#tab-accordion" href="#collapse' + n + '" aria-expanded="false" aria-controls="collapse' + n + '">' + val.innerText + '</a></h4>';
			concat += '</div>';
