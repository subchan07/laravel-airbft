(function ($) {
    ...
    // Mengambil token CSRF dari tag meta
    const csrfToken = $('meta[name="csrf-token"]').attr('content');

    // Menyisipkan token CSRF dalam setiap permintaan Ajax
    $.ajaxSetup({
        cache: false,
        headers: {
...
// Add article
function formNewArticle(event, form) {
    ...
    $.ajax({
        ...
        success: function (response) {
            hideLoadingButton(`#${form.id} [type=submit]`, 'Submit')
...
/* Fungsi formatRupiah */
function convertRupiahJs(input, field) {
    const fieldTarget = document.querySelector(field)
    if (input.value === '') {
        fieldTarget.innerHTML = ''
    } else {
        fieldTarget.innerHTML = setToRupiahJs(input.value)
    }
}

function setToRupiahJs(number) {
...
function showLoadingButton(input) {
    const buttonTarget = document.querySelector(input);
    buttonTarget.innerHTML = '<i class="fas fa-spinner fa-pulse"></i>'
    // buttonTarget.disabled = false
    buttonTarget.setAttribute('disabled', 'disabled');

}

function hideLoadingButton(input, textButton) {
    const buttonTarget = document.querySelector(input);
    buttonTarget.innerHTML = textButton
    // buttonTarget.disabled = false
